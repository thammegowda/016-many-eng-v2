...
def read_raw_parallel_lines(*streams):
    ...
    for lines in tqdm(itertools.zip_longest(*streams), desc="reading"):
...
class Augmentor:
    ...
    def inp_recs(self):
        if not self._inp_recs:
            log.warning(f"Going to buffer data; this may consume all the memory and crash. Current usage={max_RSS()[1]}.")
